...
You can use typedef to make Align is a long type, which means the maximal size is 8 bytes, so, we can have 8 bytes, 16 bytes,
32 bytes... It is good to implement the malloc function.
...
**Write a 1-2 sentence answer here:**
locks pages in the address range starting at addr and continuing for len bytes. All pages that contain a part of the specified address range are guaranteed to be resident in RAM when the call returns successfully; the pages are guaranteed to stay in RAM until later unlocked.

If we use mlock, We won't be locking massive amounts of data to RAM, and therefore won't be disrupting other processes on the system.

We can mlock() specific pieces of sensitive data to keep it safe.
...
For now the best answer is Linklist. 
...
No, we don't think so, valgrind can detect the memory leak if we use the bulit-in malloc and free. Since people can use differnt data sturecture and algorithms to implement the custom allocator, it is no possiable to detect memory leak using 
valgrind.
...
**Write a 1-2 sentence answer here:**
Ther are 25 sbrk syscalls if use strace to check.

...
//Put this function on the top so we can use it everywhere.
...
queue_t* create_queue(unsigned int _capacity){
	...
	myQueue->back = myQueue->capacity - 1;
...
int queue_dequeue(queue_t *q){
	...
	int itemfront = q->data[q->front];
	q->front = (q->front + 1)%q->capacity;
	q->size = q->size - 1;


		return itemfront; // Note: This line is a filler so the code compiles.
}

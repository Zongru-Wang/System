...
Stacks are last in first out.  So, it is good to use it as a undo/redo feature. If a user enter something, save it on a stack, and if user need to undo something, just pop it off the top of the undo stack and push it to redo stack. If we have mass data or process, such as a database, using this data structure and help us keep track the state of the database and help to back up. If the database crushed, we can make it go back to the previous states. 

It also good for reverse things. If we have a string that construct be chars in stack. (Stack chars to make them as a string). Then, if we want to reverse this string, we can just keep popping it,  and since stack is last in first out, doing pops will reverse the string. 

And since stack is last in and first out, no one can easily the data in the middle, such as insert. And using stack don’t need you to read and delete an object, which decrease the mistakes.
...
However, it is costly when we try to copy/paste the data in ordered in a stack, such as if we have a stack that represents a string, if we want to copy that string in the right order, we need to pop it all, and reverse the order, 

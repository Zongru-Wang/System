...
We need to recliam memory for the child process as well.
...
The difference is that the vfork can access the same address spaces of its parent processes, but fork is separate address space.
...
Yes, we need to use lock function in this solution, since we need to ensure that a row con only be painted by a onlt Artist.
Since we are painting in pararell, we need to ensure that two threads will not affect with each other.

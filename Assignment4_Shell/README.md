...
time(It is actually a gusses number game)



SYNOPSIS: 

command on the right side of the arrow is the usage
...
cd commands: (1) You can directly enter "cd" to go to the home directory.
             (2) Or use "cd .." to return to the upper directory. 
             (3) Or use cd <dir> to get to the further directory.



time command: It is a small "gusses number" game, it has stroies and shows the current date.
              Type time to play the game. You will have 8 chance per game. Have fun!
	      Tell user if the input is too high or too low so user can adjust their guess.
After 8 times and if user still can't get the answer, the game will quit, The user needs to 
languch the game use command "time" again to play it.
 

NOTES:
The built-in command is a guess number game. Guess a randomly generated int within 1 -10.
User has 8 chances, and it has a small story inside. I also shows the current time for user.
Use time.h library to generate a random int and mod 10. It is the answer.

And an int define how many chance for each play.
an int to define the failure = 0
each failure will add the failure to 1.

when the failure reaches to the chance,
the game will quit.
